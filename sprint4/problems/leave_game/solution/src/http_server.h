...
public:
    ...
    std::string_view GetRemoteIP() {
        std::string ip_str = stream_.socket().remote_endpoint().address().to_string();
        std::string_view ip_sv(ip_str);
        return ip_sv;
    }
protected:
...
private:
    ...
    void HandleRequest(HttpRequest&& request) override {
        ...
        std::string_view ip_sv = GetRemoteIP();

        request_handler_(ip_sv, std::move(request), [self = this->shared_from_this()](auto&& response) {
